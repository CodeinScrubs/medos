...
| C05 | Call workflow | Quick post-call note + user-selected audio import first; investigate automatic two-sided call recording on actual target hardware separately. Do not advertise ordinary microphone permission as call-recording support. **2026-09-26 (0.11.0):** recording is the Samsung dialer's own (both sides, `Recordings/Call`; the owner's firmware has the setting, off at the time). MedOS lists that folder through a one-time SAF grant, files a recording under a chosen patient as a dated «پیگیری تلفنی» note with the audio copied in, and counts unfiled recent calls on Today; any audio file can be picked instead (`features/calls`, architecture.md). Remaining: the owner turning on the dialer's recording, and a real recorded call filed on the phone. |
