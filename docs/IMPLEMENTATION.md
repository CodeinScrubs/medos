...
|---|---|---|
| W01 | Inbox and task retrieval | Implemented retrieval slice: full matching totals, search and load-more for inbox/filed captures/tasks; patient/global task editor and completed/cancelled/deleted lists; reopen and restore; capture links open the actual task. Task text/outcome edits autosave with guarded exit. SQLite tests cover overflow, ordering, partial edits, restore and write failures. Remaining: device navigation/large-list performance, quick-add draft recovery and broad search coverage. |
...
| W03 | Priorities and due work | Task high/normal/low and consult emergency/urgent/routine now use explicit SQL ranks with regression tests. Known task deadlines sort before undated tasks within a rank; closed history uses completion date. Task priority is editable. Remaining: task deadline editing and reminders. Before expanding date inputs, address QuickDateField/JalaliDateField invalid text retaining an older valid value without notifying the caller. |
