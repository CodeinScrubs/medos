 ...
 * file manager, and down to the second, so two backups never share a name.
 * Names sort in time order, which is what rotation relies on.
 */
export function backupFileName(now: Date, full: boolean): string {
  const { jy, jm, jd } = toJalali(now);
  const pad = (n: number) => String(n).padStart(2, '0');
  const time = `${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}`;
  return `MedOS-${jy}-${pad(jm)}-${pad(jd)}_${time}-${full ? 'full' : 'db'}.medosbak`;
}

/** Matches the names `backupFileName` produces, and those of earlier builds. */
export const BACKUP_FILE_RE = /^MedOS-\d{4}-\d{2}-\d{2}_\d{4,6}-(full|db)\.medosbak$/;
